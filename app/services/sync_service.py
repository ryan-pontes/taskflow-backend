from app.integrations.clickup import clickup_client, map_task_to_clickup, map_task_from_clickup
from app.integrations.supabase import get_space, create_task, update_task, get_tasks_by_space


async def maybe_sync_to_clickup(task: dict) -> dict | None:
    ...
    try:
        ...
        result = await clickup_client.create_task(list_id, clickup_data)
        
        # Atualizar tarefa local com ID do ClickUp
...
async def sync_from_clickup(space_id: str) -> list:
    ...
    try:
        clickup_tasks = await clickup_client.get_tasks(list_id)
...
async def sync_status_to_clickup(task_id: str, new_status: str):
    ...
    try:
        await clickup_client.update_task(task["clickup_id"], {"status": new_status})
    except Exception as e:
